...
**Jan 4:** Onboarding flow reordered and simplified - Personal connection first (Name → Units → Height/Weight → Gender/Age → Wearables), then HealthKit at end for sleep history sync. New Units step shows locale-detected measurement preference. Removed chronotype analysis from onboarding entirely (will add later in core questions). HealthKit step now just requests permissions, fetches demographics, and syncs sleep data in background - no analysis UI or "wow moments". Fixed `markHealthKitConnected()` timing bug - now called AFTER demographics are fetched. Added missing data banner when HealthKit can't provide some info (e.g., "We'll ask about height, weight next").
